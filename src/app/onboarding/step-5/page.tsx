...
const transformProfileData = (onboardingData: OnboardingData) => {
  ...
  const profileData = {
    ...
    skills: onboardingData.skills.skills || [],
    categories: onboardingData.professional.categories || [],
    experienceLevel: onboardingData.professional.experienceLevel,
    ...
    education: (onboardingData.education?.items || []).map((edu: any) => ({
    ...
    certifications: (onboardingData.skills?.certifications || []).map((cert: any) => ({
      name: cert.name,
        ...
        >
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center justify-between">
                <span className="text-sm font-medium text-gray-700">Step 4: Education (Optional)</span>
                <div className="flex items-center">
                  {onboardingData.education?.items?.length > 0 ? (
                    <CheckCircle className="h-5 w-5 text-green-500" />
                  ) : (
                    <div className="h-5 w-5 border-2 border-gray-300 rounded-full"></div>
                  )}
            ...
            <div className="grid md:grid-cols-2 gap-6">
              ...
              {onboardingData.professional && (
                <div className="space-y-3">
                  ...
                  <div className="space-y-2 text-sm text-gray-600 font-inter">
                    ...
                    <div><strong>Experience Level:</strong> {onboardingData.professional.experienceLevel}</div>
                    <div><strong>Categories:</strong> {onboardingData.professional.categories?.join(', ')}</div>
                    <div><strong>Hourly Rate:</strong> ${onboardingData.professional.hourlyRate}/hr</div>
                    <div><strong>Availability:</strong> {onboardingData.professional.availability?.status}</div>
                  </div>
                </div>
              )}
              ...
              {onboardingData.skills && (
                <div className="space-y-3">
                  ...
                  <div className="space-y-2 text-sm text-gray-600 font-inter">
                    <div><strong>Skills:</strong> {onboardingData.skills.skills?.join(', ')}</div>
                    <div><strong>Certifications:</strong> {onboardingData.skills.certifications?.map((c: any) => c.name).join(', ')}</div>
                  </div>
                </div>
              )}

              {/* Education */}
              {onboardingData.education && onboardingData.education.items && onboardingData.education.items.length > 0 && (
                <div className="space-y-3">
                  <h4 className="font-medium text-gray-900 font-poppins">Education</h4>
                  <div className="space-y-2 text-sm text-gray-600 font-inter">
                    {onboardingData.education.items.slice(0, 2).map((edu: any, index: number) => (
                      <div key={index}>
                        <strong>{edu.degree}</strong> - {edu.institution} ({edu.fieldOfStudy})
                      </div>
                    ))}
                    {onboardingData.education.items.length > 2 && (
                      <div className="text-xs text-gray-500">
                        +{onboardingData.education.items.length - 2} more
                      </div>
                    )}
                  </div>
                </div>
