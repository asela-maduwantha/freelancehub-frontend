...
export default function ContractMilestonesPage() {
  ...
  const isOverdue = (milestone: MilestoneResponse) => {
    return new Date(milestone.dueDate) < new Date() && milestone.status !== 'approved' && milestone.status !== 'completed';
  };
        ...
        {milestones.length === 0 ? (
          <div className="bg-white rounded-lg shadow">
            <div className="p-6">
              <div className="text-center py-12">
                ...
                <h3 className="mt-2 text-sm font-medium text-primary">No milestones found</h3>
                <p className="mt-1 text-sm text-muted">This contract doesn't have any milestones yet.</p>
              </div>
            </div>
          </div>
        ) : (
          <div className="grid gap-6">
            {milestones.map((milestone) => (
              <Card key={milestone.id} variant="default" className="w-full">
                <CardHeader>
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex items-center gap-4 text-sm text-secondary">
                        <div className="flex items-center gap-1">
                          ...
                          {getDaysUntilDue(milestone) <= 7 && getDaysUntilDue(milestone) > 0 && (
                            <span className="text-warning font-medium ml-1">
                              ({getDaysUntilDue(milestone)} days left)
                            </span>
                          )}
                          {getDaysUntilDue(milestone) === 0 && (
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4 text-sm">
                      ...
                      <div>
                        ...
                        <div className="text-secondary">{formatDate(milestone.updatedAt)}</div>
                      </div>
                    </div>
                      ...
                      <div className="border-t border-light pt-4">
                        ...
                        <div className="space-y-2">
                          {milestone.deliverables.map((deliverable: any, index: number) => (
                            <div key={`${milestone._id}-deliverable-${index}`} className="flex items-center gap-3 p-2 bg-secondary rounded-lg">
                              ...
                              <div className="flex-1 min-w-0">
                                {deliverable.url ? (
                                  <a
                                    ...
                                    className="text-primary hover:text-blue-600 hover:underline cursor-pointer transition-colors font-medium"
                                  >
                                    {deliverable.filename}
                                  </a>
                                ) : (
                                  <span className="text-primary font-medium">{deliverable.filename}</span>
                                )}
                                <div className="text-xs text-muted">
                                  {(deliverable.size / 1024).toFixed(1)} KB • {deliverable.type}
                                </div>
                              </div>
                            </div>
                          ))}
                    ...
                    {milestone.status === 'rejected' && (
                      <div className="border-t border-light pt-4">
                        <div className="alert-error rounded-lg p-3">
                          <h5 className="font-medium text-error mb-1">Rejection Feedback</h5>
                          <p className="text-sm text-error">This milestone was rejected. Please review the feedback and resubmit.</p>
                        </div>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="flex gap-3">
                    {canStartWork(milestone) && (
                      <Button
                        variant="primary"
                        size="sm"
                        onClick={() => handleStartWork(milestone.id)}
